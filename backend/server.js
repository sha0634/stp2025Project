...
app.get('/api/recipes/search', async (req, res) => {
  try {
    ...
    const response = await fetch(
      `${SPOONACULAR_BASE_URL}/findByIngredients?ingredients=${encodeURIComponent(ingredients)}&number=${number}&apiKey=${SPOONACULAR_API_KEY}`
    );
...
app.listen(PORT, () => {
  ...
  if (!SPOONACULAR_API_KEY) {
    console.warn('Warning: SPOONACULAR_API_KEY not set. Please add it to your .env file.');
  }
});